...
export class ProfessorsService {
  ...
  async showResults(id: string) {
    ...
    const rateYourProfessor = { twenty: 0, fourty: 0, sixty: 0, eighty: 0, onehundred: 0 }
    const difficulty = { _twenty: 0, _fourty: 0, _sixty: 0, _eighty: 0, _onehundred: 0 }
    ...
    professor.map((question) => {
      ...
      if (question.questionType === "difficult") {
        if (question.response === 20) {
          ...
          console.log('erzzzzzzzzzzzzzzzzzz')
    ...
    const quality = (rateYourProfessor.twenty + rateYourProfessor.fourty + rateYourProfessor.eighty + rateYourProfessor.sixty + rateYourProfessor.onehundred) / 5
    const numberOfRatings = (rateYourProfessor.twenty + rateYourProfessor.fourty + rateYourProfessor.eighty + rateYourProfessor.sixty + rateYourProfessor.onehundred)
    return ({
      rateMyTeacher: {
  ...
  async findByName(professorName: string) {
    return await this.professorModel.find({ professorName: { $regex: new RegExp(professorName, 'i') } }).exec();
  }
